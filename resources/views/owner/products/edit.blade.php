{{-- create.blade.php からコピー --}}
{{-- shops/edit を見比べながら書いていく --}}
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="post" action="{{ route('owner.products.update', ['product' => $product->id]) }}" >
                        ...
                        <div class="-m-2">
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                                    <label for="name" class="leading-7 text-sm text-gray-600">商品名 *必須</label>
                                                                                {{-- すでにDBに情報があるので、$product->name でDBの情報を持ってこれる--}}
                            ...
                            {{-- 現在の在庫を表示しつつ、増やすか減らすかの選択肢を設けて、増やしたい数減らしたい数を入れる --}}
                            {{-- 現在の在庫は編集はできない状態にしたい 値としては持っておいて次のupdateに渡したい --}}
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                                    <label for="current_quantity" class="leading-7 text-sm text-gray-600">現在の在庫</label>
{{-- 値としては持っておいて次のupdateに渡したいので、type="hidden" とする --}}                       {{-- コントローラから渡ってきた現在の在庫数 $quantity --}}
                                    ...
                                    text-base outline-none text-gray-700 py-1 px-3 leading-8    {{-- 上からコピー --}}
                                    ">{{ $quantity }}</div>
                                        {{-- classは下から持ってきた --}}
                                </div>
                            </div>
                             {{-- 数量を増やすか減らすかの選択肢を持たせたい
                                下の方に販売中かどうかのradioボタンがあるので、それを入力する --}}
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    {{-- <div><input type="radio" name="type" value="1" checked>追加</div>
                                    <div><input type="radio" name="type" value="2">削減</div> --}}
                                    <div><input type="radio" name="type" value="{{ Constant::PRODUCT_LIST['add'] }}" checked>追加</div>
                                    <div><input type="radio" name="type" value="{{ Constant::PRODUCT_LIST['reduce'] }}">削減</div>
                                </div>
                            </div>
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                                    ...
                                    {{-- input type="number" だとマイナスの数値も入ってくる可能性があるある 注意事項を書いておく --}}
                            ...
                            <x-select-image name="image1" currentId="{{ $product->image1 }}" currentImage="{{ $product->imageFirst->filename ?? '' }}" :images="$images"/>
                    ...
                    <form id="delete_{{ $product->id }}" method="post" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}">
                        ...
                        <div class="p-2 w-full flex justify-around mt-32"> {{-- *から --}}
                            <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="flex mx-auto text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">削除する</a>
                        </div>
                    </form>
                </div>
            </div>
